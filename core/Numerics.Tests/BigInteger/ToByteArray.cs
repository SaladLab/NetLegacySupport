// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.
// See the LICENSE file in the project root for more information.

using NUnit.Framework;

namespace System.Numerics.Tests
{
    public class ToByteArrayTest
    {
        [Theory]
        [TestCase(0, new byte[] { 0x00 })]
        [TestCase(3, new byte[] { 0x03 })]
        [TestCase(128, new byte[] { 0x80, 0x00 })]
        [TestCase(200, new byte[] { 0xc8, 0x00 })]
        [TestCase(256, new byte[] { 0x00, 0x01 })]
        [TestCase(2005, new byte[] { 0xd5, 0x07 })]
        [TestCase(10197, new byte[] { 0xd5, 0x27 })]
        [TestCase(33023, new byte[] { 0xff, 0x80, 0x00 })]
        [TestCase(2368349, new byte[] { 0x5d, 0x23, 0x24 })]
        [TestCase(10756957, new byte[] { 0x5d, 0x23, 0xa4, 0x00 })]
        [TestCase(193100307, new byte[] { 0x13, 0x7a, 0x82, 0x0b })]
        [TestCase(1266842131, new byte[] { 0x13, 0x7a, 0x82, 0x4b })]
        [TestCase(int.MaxValue, new byte[] { 0xff, 0xff, 0xff, 0x7f })]
        [TestCase(-1, new byte[] { 0xff })]
        [TestCase(-128, new byte[] { 0x80 })]
        [TestCase(-172, new byte[] { 0x54, 0xff })]
        [TestCase(-23439, new byte[] { 0x71, 0xa4 })]
        [TestCase(-51301, new byte[] { 0x9b, 0x37, 0xff })]
        [TestCase(-126341, new byte[] { 0x7b, 0x12, 0xfe })]
        [TestCase(-13194515, new byte[] { 0xed, 0xaa, 0x36, 0xff })]
        [TestCase(-2068145902, new byte[] { 0x12, 0x99, 0xba, 0x84 })]
        [TestCase(int.MinValue, new byte[] { 0x00, 0x00, 0x00, 0x80 })]
#if NET20
        public void FromIntTests_NET20(int i, byte[] expectedBytes)
#elif NET35
        public void FromIntTests_NET35(int i, byte[] expectedBytes)
#elif NET40
        public void FromIntTests_NET40(int i, byte[] expectedBytes)
#endif
        {
            BigInteger bi = new BigInteger(i);
            byte[] bytes = bi.ToByteArray();
            Assert.AreEqual(expectedBytes, bytes);
            BigInteger bi2 = new BigInteger(bytes);
            Assert.AreEqual(bi, bi2);
        }

        [Theory]
        [TestCase(0x100112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x01 })]
        [TestCase(0x300112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x03 })]
        [TestCase(0x8000112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x80, 0x00 })]
        [TestCase(0x3cd00112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0xcd, 0x03 })]
        [TestCase(0xf92100112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x21, 0xf9, 0x00 })]
        [TestCase(0x749aa00112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0xaa, 0x49, 0x07 })]
        [TestCase(0x80112200112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x22, 0x11, 0x80, 0x00 })]
        [TestCase(0x7654321000112233L, new byte[] { 0x33, 0x22, 0x11, 0x00, 0x10, 0x32, 0x54, 0x76 })]
        [TestCase(long.MaxValue, new byte[] { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f })]
        [TestCase(-0x100112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0xfe })]
        [TestCase(-0x8000112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0x7f, 0xff })]
        [TestCase(-0x3cd00112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0x32, 0xfc })]
        [TestCase(-0xf92100112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0xde, 0x06, 0xff })]
        [TestCase(-0x749aa00112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0x55, 0xb6, 0xf8 })]
        [TestCase(-0x80112200112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0xdd, 0xee, 0x7f, 0xff })]
        [TestCase(-0x7654321000112233L, new byte[] { 0xcd, 0xdd, 0xee, 0xff, 0xef, 0xcd, 0xab, 0x89 })]
        [TestCase(long.MinValue, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80 })]
        [TestCase(-0x100000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0xff })]
        [TestCase(-0x300000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0xfd })]
        [TestCase(-0x8000000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x80 })]
        [TestCase(-0xfe00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x02, 0xff })]
        [TestCase(-0xff00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x01, 0xff })]
        [TestCase(-0xfeff00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0xff })]
        [TestCase(-0xffff00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0xff })]
        [TestCase(-0xfeffff00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0xff })]
        [TestCase(-0xffffff00000000L, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0xff })]
#if NET20
        public void FromLongTests_NET20(long l, byte[] expectedBytes)
#elif NET35
        public void FromLongTests_NET35(long l, byte[] expectedBytes)
#elif NET40
        public void FromLongTests_NET40(long l, byte[] expectedBytes)
#endif
        {
            BigInteger bi = new BigInteger(l);
            byte[] bytes = bi.ToByteArray();
            Assert.AreEqual(expectedBytes, bytes);
            BigInteger bi2 = new BigInteger(bytes);
            Assert.AreEqual(bi, bi2);
        }

        [Theory]
        [TestCase("-18374686475376656384", new byte[] { 0xff, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00 })]
        [TestCase("-18446744069414584320", new byte[] { 0xff, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00 })]
        [TestCase("", new byte[] {  })]
        [TestCase("-", new byte[] {  })]
#if NET20
        public void FromStringTests_NET20(string str, byte[] expectedBigEndianBytes)
#elif NET35
        public void FromStringTests_NET35(string str, byte[] expectedBigEndianBytes)
#elif NET40
        public void FromStringTests_NET40(string str, byte[] expectedBigEndianBytes)
#endif
        {
            byte[] expectedBytes = new byte[expectedBigEndianBytes.Length];
            for (var i = 0; i < expectedBigEndianBytes.Length; i++)
            {
                expectedBytes[i] = expectedBigEndianBytes[expectedBigEndianBytes.Length - 1 - i];
            }

            BigInteger bi = BigInteger.Parse(str);
            byte[] bytes = bi.ToByteArray();
            Assert.AreEqual(expectedBytes, bytes);
            BigInteger bi2 = new BigInteger(bytes);
            Assert.AreEqual(bi, bi2);
        }
    }
}
